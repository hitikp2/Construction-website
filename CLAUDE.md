...
- Image generation uses **Google Nano Banana 2** (`gemini-3.1-flash-image-preview`)
  via the `generateContent` endpoint with `responseModalities: ['TEXT', 'IMAGE']`.
  Uses the same `GEMINI_API_KEY`. Do NOT use older Imagen models.
